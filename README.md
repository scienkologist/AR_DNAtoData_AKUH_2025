...
- Use downstream tools like:
    - **Dorado**
        Part	Description
{Filepath of extracted dorado}	The location on your system where the Dorado tool was extracted. Replace this with the actual path (e.g., C:\Users\You\Downloads\dorado-windows)
\bin\dorado.exe	This is the actual Dorado executable used to perform the basecalling. It’s located in the bin folder within the Dorado directory.
basecaller	This tells Dorado to perform basecalling, converting raw signal data (from .pod5 files) into nucleotide sequences (FASTQ format).
--device cpu	Specifies to run the basecaller on the CPU instead of a GPU (useful if you don’t have CUDA or a GPU available).
--models-directory .\models\	This option sets the directory where models are stored. Dorado needs a trained model for basecalling.
.\models\dna_r9.4.1_e8_sup@v3.6\	This is the specific model directory you are using for the basecalling. In this case, it’s the super accuracy model for R9.4.1 flow cells and E8 chemistry (v3.6).
.\pod5\	This is the input folder that contains the .pod5 raw signal files generated by the Nanopore sequencer.
> out2.fastq	Redirects the basecalled output to a file named out2.fastq in the current directory. This file will contain the nucleotide sequences and quality scores in FASTQ format.
